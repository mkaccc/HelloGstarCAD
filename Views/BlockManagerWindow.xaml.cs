...
namespace HelloGstarCAD.Views
{
    public partial class BlockManagerWindow : Window
    {
        ...
        private void BtnStartNumbering_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("文字高度必须是一个有效的正数。", "输入错误", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            if (!double.TryParse(TxtOffsetX.Text, out double offsetX))
            {
                MessageBox.Show("X偏移必须是有效的数字。", "输入错误", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            if (!double.TryParse(TxtOffsetY.Text, out double offsetY))
            {
                MessageBox.Show("Y偏移必须是有效的数字。", "输入错误", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            try
            {
                ...
                var confirmResult = MessageBox.Show($"将为 {_blockItems.Count} 种图块创建沿线编号。\n" +
                                                   ...
                                                   $"文字高度: {textHeight}, 图层: {TxtLayerName.Text}\n" +
                                                   $"X偏移: {offsetX}, Y偏移: {offsetY}\n\n是否继续？", 
                ...
                if (confirmResult == MessageBoxResult.Yes)
                {
                    ...
                    string textStyle = TxtTextStyle.Text;
                    bool autoCreateLayer = ChkAutoCreateLayer.IsChecked ?? true;
                    
                    // 调用新的沿线编号方法
                    _cadService.PlaceNumbersAlongPolyline(
                        _selectedPolylineId.Value, 
                        _blockItems.ToList(), 
                        prefix, 
                        suffix, 
                        startNumber,
